...
const TaskListNumbers = ({ taskList }) => {
  ...
  useEffect(() => {
    // This function updates task counts based on the task list
    ...
    // Update task counts when taskList prop changes
    if (taskList) {
      updateTaskCounts(taskList);
    }
  }, [taskList]); // Trigger re-run when taskList prop changes
